...
Используй команду /info чтобы узнать команды""",
...
Также ты можешь ввести имя проекта и узнать информацию о нём!"""
    )
...
def callback_project(message, data, statuses):
    ...
    if status not in statuses:
        bot.send_message(
            message.chat.id,
            "Ты выбрал статус не из списка, попробуй ещё раз!",
            reply_markup=gen_markup(statuses)
        )
        bot.register_next_step_handler(
            message,
...
def skill_project(message, projects):
    ...
    if project_name not in projects:
        bot.send_message(
            message.chat.id,
            'У тебя нет такого проекта, попробуй ещё раз! Выбери проект, для которого нужно выбрать навык',
            reply_markup=gen_markup(projects)
        )
        bot.register_next_step_handler(message, skill_project, projects=projects)
    else:
...
def set_skill(message, project_name, skills):
    ...
    if skill not in skills:
        bot.send_message(
            message.chat.id,
            'Видимо, ты выбрал навык не из списка, попробуй ещё раз! Выбери навык',
            reply_markup=gen_markup(skills)
        )
        bot.register_next_step_handler(
            message,
...
def delete_handler(message):
    ...
    if projects:
        ...
        bot.send_message(
            message.chat.id,
            "Выбери проект, который хочешь удалить:\n\n" + text,
            reply_markup=gen_markup(projects_list)
        )
        bot.register_next_step_handler(
            message,
            delete_project,
...
def delete_project(message, projects):
    ...
    if project not in projects:
        bot.send_message(
            message.chat.id,
            'У тебя нет такого проекта, попробуй выбрать ещё раз!',
            reply_markup=gen_markup(projects)
        )
        bot.register_next_step_handler(
            message,
...
def update_project_step_2(message, projects):
    ...
    if project_name not in projects:
        bot.send_message(
            message.chat.id,
            "Что-то пошло не так! Выбери проект, который хочешь изменить ещё раз:",
            reply_markup=gen_markup(projects)
        )
        bot.register_next_step_handler(
            message,
...
def update_project_step_3(message, project_name):
    ...
    if attribute not in attributes_of_projects.keys():
        bot.send_message(
            message.chat.id,
            "Кажется, ты ошибся, попробуй ещё раз!",
            reply_markup=gen_markup(attributes_of_projects.keys())
        )
        bot.register_next_step_handler(
            message,
...
def update_project_step_4(message, project_name, attribute):
    ...
    if attribute == "status_id":
        ...
        if update_info in status_names:
            ...
            bot.send_message(
                message.chat.id,
                "Был выбран неверный статус, попробуй ещё раз!",
...
def text_handler(message):
    ...
    bot.reply_to(
        message,
        "Тебе нужна помощь? Используй команду /info"
    )
